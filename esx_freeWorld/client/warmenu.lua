...
function WarMenu.CreateMenu(id, title, subTitle)
    ...
    menus[id].titleBackgroundColor = { r = 33, g = 161, b = 255, a = 255 }
    menus[id].menuTextColor = { r = 255, g = 255, b = 255, a = 255 }
    menus[id].menuSubTextColor = { r = 0, g = 119, b = 255, a = 255 }
...
function WarMenu.Visible(toggle)
    if menus[currentMenu] then
        setMenuVisible(currentMenu, toggle)
    end
end
...
function WarMenu.Display()
    if isMenuVisible(currentMenu) then
        if menus[currentMenu].aboutToBeClosed then
            ...
            elseif IsControlPressed(0, keys.up) and (GetGameTimer() - GUI.Time) > 150 then
				GUI.Time = GetGameTimer()
                PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", true)
