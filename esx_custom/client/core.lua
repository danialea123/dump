---@diagnostic disable: undefined-field, missing-parameter, lowercase-global, undefined-global
local uiOpen = false
...
function openUI(index, Command)
    if (uiOpen) then return end
    if ESX.GetPlayerData().job.name ~= "mechanic" and ESX.GetPlayerData().job.name ~= "benny" and ESX.GetPlayerData().job.name ~= "rioters" and ESX.GetPlayerData().permission_level < 10 then return end
    if ESX.GetPlayerData().job.grade < 5 and ESX.GetPlayerData().permission_level < 10 then return ESX.Alert("Shoma Customer Nisitd", "info") end
    local playerPed = PlayerPedId()
    local playerVeh = GetVehiclePedIsIn(playerPed, false)
    
    if (playerVeh ~= 0 and DoesEntityExist(playerVeh)) then
        local Class = GetVehicleClass(playerVeh)
        if not Command then
            if List["benny"][Class] then
                if ESX.GetPlayerData().job.name ~= "benny" then
                    return ESX.Alert("In Mashin Faghat Dar Mechanici Benny Custom Mishavad", "error")
                end
            end
            if List["mechanic"][Class] then
                if ESX.GetPlayerData().job.name ~= "mechanic" then
                    return ESX.Alert("In Mashin Faghat Dar Mechanici Central Custom Mishavad", "error")
                end
            end
        end
        GangHeli = false
        local partUpgrade = isVehicleOwner(GetVehicleData(playerVeh), Command)
        if not partUpgrade then
            partUpgrade = isVehicleGang(GetVehicleData(playerVeh), Command)
            if partUpgrade then
                GangHeli = true
            end
        end
...
function closeUI(sendToUI, resetVehToDefault)
    ...
    --if not isCommand then
    if not GangHeli then
        TriggerServerEvent('irrp_custom:customMenuClosed', VehToNet(customVehicle), ownerVeh)
    end
    --end
    customVehicle = nil
    customVehicleData = nil
                    ...
                    elseif (menuOption.price > 0) then
                        ...
                        if (getCurrentMoney() >= tempPrice) then
                            ...
                            if (tempPrice > 0 or Config.AutoSaveVehiclePropertiesOnApply == true) then
                                ...
                                if not GangHeli then
                                    TriggerServerEvent('irrp_custom:requestUpgrade', {price = tempPrice, upgrades = vehiclePorperties}, ownerVeh, isCommand)
                                else
                                    TriggerServerEvent("esx_gangs:setHeliPropsMC", ownerVeh, heliID, vehiclePorperties)
                                end
                        ...
                        if (getCurrentMoney() >= tempPrice) then
                            if (tempPrice > 0 or Config.AutoSaveVehiclePropertiesOnApply == true) then
                                if type(data.color) ~= "number" then
                                    if ucolor[1] == data.color[1] and ucolor[2] == data.color[2] and ucolor[3] == data.color[3] then
                                        playSound('ATM_WINDOW', 'HUD_FRONTEND_DEFAULT_SOUNDSET')
                                        return
                                    end
                                end
    ...
    for i = startIndex, GetNumVehicleModData(customVehicle, menuOption.modType), 1 do
        ...
        elseif (menuOption.modType == 'extraEnabled') then
            ...
            tempOption.onSelect = function()
                local class = GetVehicleClass(customVehicle)
                print(class)
...
function isVehicleGang(default, index)
    local p = promise.new()
    ESX.TriggerServerCallback('esx_gangProp:DoesPlateExists', function(owner, veh, id)
        ownerVeh = veh ~= nil and veh
        heliID = id
        p:resolve(owner)
    end, default, index)
...
function isVehicleOwner(default, index)
    local p = promise.new()
    ESX.TriggerServerCallback('irrp_custom:isVehicleEligable', function(owner, veh)
        ownerVeh = veh ~= nil and veh
        p:resolve(owner)
    end, default, index)
    return Citizen.Await(p)
...
function toggleHud(status)
	...
	--exports.esx_streetlabel:Hide(status)
...
Citizen.CreateThread(function()
    while true do
        for _, vehicle in ipairs(GetGamePool('CVehicle')) do
            if GetEntityPopulationType(vehicle) ~= 7 then goto skip_vehicle end

            local hasControl = NetworkHasControlOfEntity(vehicle)
            if not hasControl then goto skip_vehicle end

            local veh = Entity(vehicle)
            local canBurst = GetVehicleTyresCanBurst(vehicle)
            local plate = ESX.Math.Trim(GetVehicleNumberPlateText(vehicle))

            if canBurst and veh.state.bproof then
                SetVehicleTyresCanBurst(vehicle, false)
            elseif not canBurst and not veh.state.bproof then
                SetVehicleTyresCanBurst(vehicle, true)
            end

            if veh.state.plate and plate ~= veh.state.plate then
                SetVehicleNumberPlateText(vehicle, veh.state.plate)
            end
...
RegisterNetEvent("esx_customer:RollBackCustom")
AddEventHandler("esx_customer:RollBackCustom", function(id, cus)
	local ent = nil
    for k, v in pairs(GetGamePool('CVehicle')) do
        if ESX.Math.Trim(GetVehicleNumberPlateText(v)) == ESX.Math.Trim(id) then
            ent = v
        end
    end
	if ent then
        ...
        ESX.Game.SetVehicleProperties(ent, cus)
	end
end)