...
furni.awake = function()
  ...
  if Configs.UsingESX then
    while not ESX                   do Citizen.Wait(0); end
    while not ESX.GetPlayerData().job  do Citizen.Wait(0); end

    furni.plyData = ESX.GetPlayerData()

    if Configs.UsingKashacters then
        ...
        elseif furni.doingRemoval then
          ...
          if ent and ent ~= -1 and ent ~= 0 then
            ...
            if ePos and ePos.x and ePos.x ~= 0 and ePos.x ~= 0.0 then
              local model = GetEntityModel(ent)
              if not furni.antiGrab[model] then
                Utils.drawText3D(pos,"~y~Hold [~s~LMB/RT~y~] to sell [~s~"..(hashtable[model] and hashtable[model].name or "Unknown").."~y~] [~s~"..(hashtable[model] and "$"..math.floor(hashtable[model].price*(Configs.ResaleValue and Configs.ResaleValue/100.0 or 0.5)) or "Unknown").."~y~]~s~",1,7)
                if IsDisabledControlPressed(0, Controls["Grab"]) or IsControlPressed(0, Controls["Grab"]) and not furni.focused then
                  keyHeldFor = keyHeldFor + (GetGameTimer() - lastFrame)
                  if keyHeldFor >= 500 then
                    keyHeldFor = 0
...
furni.edit = function(ent,data)
  ...
  while DoesEntityExist(ent) do
    ...
    ESX.SetEntityCoords(ent, 0, 0, 0)
    SetEntityAsMissionEntity(ent,true,true)
    DeleteEntity(ent)
    DeleteObject(ent)
    Citizen.CreateThread(function()
      ESX.Game.DeleteLocalObject(ent)
...
furni.doCancel = function(...) 
  ...
  elseif furni.object then
    ...
    ESX.SetEntityCoordsNoOffset(furni.object, p.x,p.y,p.z)
...
furni.doPlace = function(...)
  ...
  if furni.inHouse then
    local cobject = furni.object
    local pos = GetEntityCoords(furni.object) - furni.inHouse.Entry.xyz - vector3(0.0, 0.0, furni.inHouse.Offset)
    local rot = GetEntityRotation(furni.object, 2)
      ...
      ESX.TriggerServerCallback('mf_housing:canAfford', function(canAfford)
        if canAfford then
          ...
          TriggerServerEvent('furni:PlaceFurniture', furni.inHouse,item,pos,rot,furni.object)
            ...
            ESX.ShowNotification("Shoma pole kafi baraye kharid in furniture ra nadarid!")
            SetEntityAsMissionEntity(cobject,true,true)
...
furni.doTransform = function(tab)
  ...
  furni.rotation = r
  ESX.SetEntityCoordsNoOffset(furni.object, Utils.round(tab.position.x,2),Utils.round(tab.position.y,2),Utils.round(tab.position.z,2))
  FreezeEntityPosition(furni.object,true)
end
...
GetPlayerCash = function()
  if Configs.UsingESX then
    ...
    if Configs["UsingESX_V1.2.0"] then
      for k,v in pairs(PlayerData.accounts) do
        if v.name == Configs.CashAccountName then
          return v.money
        end
      end
    else
...
GetPlayerBank = function()
  if Configs.UsingESX then
    PlayerData = GetPlayerData()
    for k,v in pairs(PlayerData.accounts) do
      if v.name == Configs.BankAccountName then
        return v.money
      end
    end
    return 0
