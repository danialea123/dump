  ...
  if (#vehicles > 0) then
    for _,vehData in pairs(vehicles) do
      ...
      vehicle.Activated = function(...) 
        SpawnVehicle(d.Garage, vehData.vehicle.model, vehData.vehicle, d.Id)
      end
...
NativeGarageOwnerMenu = function(d)
  ...
  if IsPedInAnyVehicle(ped,false) then
    ...
    if canStore then
      ...
      DeleteVehicle(veh)  
      TriggerServerEvent("Allhousing:VehicleStored",d.Id,props.plate)
  ...
  if IsPedInAnyVehicle(ped,false) then
    ...
    elseif Config.UsingLockpickV2 then
      ...
      if Config.UsingProgressBars then
        exports["esx_basicneeds"]:startUI(Config.LockpickTime * 1000,"Lockpicking Door")
      end
...
NativeEntryOwnerMenu = function(d)
  ...
  if d.Shell then
    ...
    enter.Activated = function(...) EnterHouse(d, false); end
    ...
    if d.Unlocked then
      local lock = NativeUI.CreateItem("Lock House",d.Shell)
      lock.Activated = function(...) LockHouse(d, false); end
      entryMenu:AddItem(lock)
    else
      local unlock = NativeUI.CreateItem("Unlock House",d.Shell)
      unlock.Activated = function(...) EnterHouse(d, false); end
  ...
  if not hasKeys then
    ...
    if d.Shell then
      ...
      if d.Unlocked then
        ...
        enterHouse.Activated = function(...) EnterHouse(d,false); end
