local Config = lib.require('sky/config')
...
local function startJump(coords, style, trail)
    TriggerEvent('esx_Quest:point', 'SkyDiving', nil, 1)
    DoScreenFadeOut(100)
    lib.requestModel(Config.HeliModel)
    lib.requestModel(Config.PilotModel)
    Wait(500)
    ESX.SetEntityCoords(cache.ped, coords.x, coords.y, coords.z+10.0)
...
local function initPed()
    ...
    if Config.UseTarget then
        exports['diamond_target']:AddTargetEntity(START_PED, {
            options = {
                {
