...
const app = new Vue({
    ...
    mounted(){
        ...
        window.addEventListener('message',function(event){
            ...
            if (event.data.action == 'Gunshow') {
                $('#wrap').show();
            }
            else if (event.data.action == 'Gunhide') {
                $('#wrap').hide();
            }
            else if (event.data.action == 'setWeaponImg'){
                //console.log(action)
                $('#weapon-img').css(`background-image`, `url(http://5.42.217.109/imgs/` + action.data + `.png)`);
                $('#bottom-left-weapon').show();
            }
            else if (event.data.action == 'updateGun'){
                $('#weapon-info').text(event.data.data)
            }
            if (action.type == "info"){
