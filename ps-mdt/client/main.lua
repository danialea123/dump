...
AddEventHandler('esx:setJob', function(job)
    PlayerData.job = job
    TriggerServerEvent("mdt:change:Job")
...
RegisterCommand('mdt', function()
    ...
    if not IsPauseMenuActive() then
        if GetJobType(PlayerData.job.name) then
...
local function EnableGUI(enable)
    ...
    SendNUIMessage({ type = "show", enable = enable, job = PlayerData.job.name, jobType = GetJobType(PlayerData.job.name), rosterLink = Config.RosterLink[PlayerData.job.name], sopLink = Config.sopLink[PlayerData.job.name] })
    isOpen = enable
    doAnimation()
end
...
local function RefreshGUI()
    ...
    SendNUIMessage({ type = "show", enable = false, job = PlayerData.job.name, jobType = GetJobType(PlayerData.job.name), rosterLink = Config.RosterLink[PlayerData.job.name], sopLink = Config.sopLink[PlayerData.job.name] })
    isOpen = false
end
...
RegisterNetEvent('mdt:client:open', function(bulletin, activeUnits, calls, cid)
    ...
    local firstname = (name:match("([^/]+)_"))
    local lastname = (name:match("_([^/]+)"))
    print(json.encode(calls))
    SendNUIMessage({ type = "data", activeUnits = activeUnits, citizenid = cid, ondutyonly = Config.OnlyShowOnDuty, name = "Welcome, " ..PlayerData.job.grade_label..' '..firstname..' '..lastname, location = playerStreetsLocation, fullname = firstname..' '..lastname, bulletin = bulletin })
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    for i=1,#vehicles do
        ...
        if hashVehicule and IsModelInCdimage(hashVehicule) then
            ...
            result.vehicles[i]['model'] = vehicleName
        else
            vehicles[i] = nil
        end
    end
...
RegisterNUICallback("searchVehicles", function(data, cb)
    ...
    for i=1, #result do
        ...
        if hashVehicule and IsModelInCdimage(hashVehicule) then
            ...
            if result[i].image == "img/not-found.webp" then
                result[i].image = "https://media.diamondrp.ir/garage/"..string.lower(GetDisplayNameFromVehicleModel(hashVehicule))..".png"
                print(result[i].image)
            end
            result[i]['model'] = vehicleName
        else
            result[i] = nil
        end
    end
...
RegisterNetEvent("mdt:client:dutystatus")
AddEventHandler("mdt:client:dutystatus", function()
...
RegisterNUICallback("toggleDuty", function(data, cb)
    TriggerServerEvent("mdt:SetDutyStatus", data.cid)
    cb(true)
end)
...
RegisterNetEvent('mdt:client:getVehicleData', function(sentData)
    if sentData and sentData[1] then
        ...
        if hashVehicule and IsModelInCdimage(hashVehicule) then
            ...
            vehicle['class'] = Config.ClassList[GetVehicleClassFromName(hashVehicule)]
            vehicle['vehicle'] = nil
            if vehicle['image'] == "img/not-found.webp" then
                vehicle['image'] = "https://media.diamondrp.ir/garage/"..string.lower(GetDisplayNameFromVehicleModel(hashVehicule))..".png"
            end
