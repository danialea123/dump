...
Emotes = {
	["cocacola"] = {"amb@world_human_drinking@coffee@male@idle_a", "idle_c", "Coca Cola", AnimationOptions =
	{
	...
	["fanta"] = {"amb@world_human_drinking@coffee@male@idle_a", "idle_c", "Fanta", AnimationOptions =
	{
	...
	["sprite"] = {"amb@world_human_drinking@coffee@male@idle_a", "idle_c", "Sprite", AnimationOptions =
	{
   ...
   ["stronzo"] = {"amb@world_human_drinking@coffee@male@idle_a", "idle_c", "Beer", AnimationOptions =
   {
...
RegisterNetEvent("esx_drink:setDrinkArmour")
AddEventHandler("esx_drink:setDrinkArmour", function()
	ESX.TriggerServerCallback("esx_drink:setDrinkArmour", function(res) 
		if res then
			ESX.SetPedArmour(PlayerPedId(), GetPedArmour(PlayerPedId()) + 25)
		end
	end)
end)

RegisterNetEvent("esx_drink:setDrinkHealth")
AddEventHandler("esx_drink:setDrinkHealth", function()
	ESX.TriggerServerCallback("esx_drink:setDrinkHealth", function(res) 
		if res then
			ESX.SetEntityHealth(PlayerPedId(), GetEntityHealth(PlayerPedId()) + 25)
		end
	end)
end)
...
AddEventHandler('esx_basicneeds:playAnim', function(name)
	...
	if name == "soda" or name == "cocacola" or name == "coffee" or name == "tea" or name == "whiskey" or name == "wine" or name == "beer" or name == "stronzo" then
...
AddEventHandler('esx_customItems:useClipcli', function()
		...
		if IsPedArmed(ped, 4) then
			...
			ESX.Alert("Shoma ba movafaghiat az kheshab estefade kardid", "check")
			else
			ESX.Alert("hash aslahe mored nazar namaloom ast", "error")
			end
			
		else
			ESX.Alert("Shoma aslaheyi dar dast nadarid", "error")
		end
...
AddEventHandler('esx_customItems:useTint', function(info)
	...
	if IsPedArmed(ped, 4) then
		...
		local currentweapon = GetSelectedPedWeapon(ped)
		  ...
		  ESX.Alert("~h~Shoma ba movafaghiat 1x ~g~" .. info.label .. " ~w~use kardid!")
...
AddEventHandler('esx_customItems:useExtendedMagazine', function()
	...
	if IsPedArmed(ped, 4) then
		...
		if extendedClips[weapon] then
			...
			ESX.Alert("In aslahe extended magazine ra support nemikonad!", "error")
		end
...
AddEventHandler('esx_customItems:useDrumMagazine', function()
	...
	if IsPedArmed(ped, 4) then
		...
		if drumMagazines[weapon] then
			...
			ESX.Alert("In aslahe Drum magazine ra support nemikonad!", "error")
		end
...
AddEventHandler('esx_customItems:useSilencer', function()
	...
	if IsPedArmed(ped, 4) then
		...
		if silencers[weapon] then
			...
			ESX.Alert("In aslahe silencer ra support nemikonad!", "error")
		end
...
AddEventHandler('esx_customItems:useFlashlight', function()
	...
	if IsPedArmed(ped, 4) then
		...
		if flashlights[weapon] then
			...
			ESX.Alert("In aslahe flashlight ra support nemikonad!", "error")
		end
...
AddEventHandler('esx_customItems:useGrip', function()
	...
	if IsPedArmed(ped, 4) then
		local selected = GetSelectedPedWeapon(GetPlayerPed(-1))
		local hash = 0
		...
		for i = 1, #weapons, 1 do
			if GetHashKey(weapons[i].name) == selected then
				for j = 1, #weapons[i].components, 1 do
					if weapons[i].components[j].name == "grip" then
		...
		if DoesWeaponTakeWeaponComponent(selected, hash) then
            ...
            TriggerServerEvent("core_weapon:removeItem", "grip", "attachment", "grip")
		else
			ESX.Alert("In aslahe grip ra support nemikonad!", "info")
		end
		
	else
		ESX.Alert("Shoma aslaheyi dar dast nadarid", "info")
	end
end)
	...
	if IsPedArmed(ped, 4) then
		...
		elseif input == "grip" then
			if grips[weapon] then
				...
				ESX.Alert("In aslahe grip ra support nemikonad!", "error")
			end
...
AddEventHandler('esx_customItems:useYusuf', function()
			...
			local currentWeaponHash = GetSelectedPedWeapon(ped)
					...
					if currentWeaponHash == GetHashKey("WEAPON_PISTOL") then
						...
						TriggerServerEvent('esx_customItems:addComponent',  { id = "luxe_varmod", weapon = "WEAPON_PISTOL", item = "yusuf"})
					elseif currentWeaponHash == GetHashKey("WEAPON_PISTOL50") then
						...
						TriggerServerEvent('esx_customItems:addComponent', { id = "luxe_varmod", weapon = "WEAPON_PISTOL50", item = "yusuf"})
					elseif currentWeaponHash == GetHashKey("WEAPON_APPISTOL") then
						...
						TriggerServerEvent('esx_customItems:addComponent', { id = "luxe_varmod", weapon = "WEAPON_APPISTOL", item = "yusuf"})
					elseif currentWeaponHash == GetHashKey("WEAPON_HEAVYPISTOL") then
						...
						TriggerServerEvent('esx_customItems:addComponent', { id = "luxe_varmod", weapon = "WEAPON_HEAVYPISTOL", item = "yusuf"})
					elseif currentWeaponHash == GetHashKey("WEAPON_SMG") then
						...
						TriggerServerEvent('esx_customItems:addComponent', { id = "luxe_varmod", weapon = "WEAPON_SMG", item = "yusuf"})
					elseif currentWeaponHash == GetHashKey("WEAPON_MICROSMG") then
						...
						TriggerServerEvent('esx_customItems:addComponent', { id = "luxe_varmod", weapon = "WEAPON_MICROSMG", item = "yusuf"})
					elseif currentWeaponHash == GetHashKey("WEAPON_ASSAULTRIFLE") then
						...
						TriggerServerEvent('esx_customItems:addComponent', { id = "luxe_varmod", weapon = "WEAPON_ASSAULTRIFLE", item = "yusuf"})
					elseif currentWeaponHash == GetHashKey("WEAPON_CARBINERIFLE") then
						...
						TriggerServerEvent('esx_customItems:addComponent', { id = "luxe_varmod", weapon = "WEAPON_CARBINERIFLE", item = "yusuf"})
					elseif currentWeaponHash == GetHashKey("WEAPON_ADVANCEDRIFLE") then
						...
						TriggerServerEvent('esx_customItems:addComponent', { id = "luxe_varmod", weapon = "WEAPON_ADVANCEDRIFLE", item = "yusuf"})
					else 
						ESX.Alert("Aslahe mored nazar ghabeliat estefade kardan az skin talaee ra nadarad")
					end
end)
...
AddEventHandler('esx_customItems:useBlowtorch', function()
            ...
            ESX.Alert("Hich mashini nazdik shoma nist")
          end
...
function checkvehicle(vehicle)
	Citizen.CreateThread(function()
		while CheckVehicle do
			...
			if vehicle ~= NearVehicle or not IsFree(vehicle) then
				ESX.Alert("Mashin Az Shoma Door Shod", "info")
