...
Citizen.CreateThread(function()
    ...
    while not ESX or not ESX.GetPlayerData().job do
        Citizen.Wait(1000)
    end
     
    PlayerData = ESX.GetPlayerData()
    PlayerData.job.type = GetJobType(PlayerData.job.name)
end)
...
AddEventHandler('esx:setJob', function(job)
    PlayerData.job = job
    PlayerData.job.type = GetJobType(job.name)
end)
...
function setupDispatch()
    ...
    PlayerData = {
        ...
        metadata = {
            callsign = GetPlayerName(PlayerId())
        },
        citizenid = playerInfo.identifier,
        job = {
            type = GetJobType(playerInfo.job.name),
...
local function isJobValid(data)
    ...
    if type(data) == "string" then
        return lib.table.contains(Config.Jobs, PlayerData.job.name)
    elseif type(data) == "table" then
        return lib.table.contains(data, PlayerData.job.name)
    end
