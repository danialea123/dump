...
local function HasPhone()
    for _, item in ipairs(Config.PhoneItems) do
        if ESX.DoesHaveItem2(item, 1) then
            return true
        end
    end
    return false
...
function IsCallAllowed(message)
    ...
    if Config.PhoneRequired and not HasPhone() then ESX.Alert('Shoma Bayad Radio Ya Gooshi Dashte Bashid', 'error') return false end
