...
$(document).keyup(function(e) {
    if (e.keyCode === 27) {
         ...
         $.post('https://esx_weaponIndicator/close', JSON.stringify({}));
    }
});
...
window.addEventListener('message', function(event) {
  ...
  $.post('https://esx_weaponIndicator/getComponent', JSON.stringify({component: 'suppressor'}));
});

    $( "#u234" ).click(function() {
   
  $("#u278").css('background', "rgba(9,9,9,0.5)");
  $.post('https://esx_weaponIndicator/getComponent', JSON.stringify({component: 'clip_extended'}));
});

    $( "#u243" ).click(function() {
   
  $("#u281").css('background', "rgba(9,9,9,0.5)");
  $.post('https://esx_weaponIndicator/getComponent', JSON.stringify({component: 'flashlight'}));
});

     $( "#u261" ).click(function() {
   
  $("#u284").css('background', "rgba(9,9,9,0.5)");
  $.post('https://esx_weaponIndicator/getComponent', JSON.stringify({component: 'luxary_finish'}));
});

      $( "#u317" ).click(function() {
   
  $("#u320").css('background', "rgba(9,9,9,0.5)");
  $.post('https://esx_weaponIndicator/getArmor', JSON.stringify({}));
});
