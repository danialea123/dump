...
local Drawables = {
	["Top"] = {
		Drawable = 11,
		Table = {Standalone = true, Male = 15, Female = 74},
	...
	["Shoes"] = {
		Drawable = 6,
		Table = {Standalone = true, Male = 107, Female = 5},
		Emote = {Dict = "random@domestic", Anim = "pickup_low", Move = 0, Dur = 1200}
	},
	["Neck"] = {
		Drawable = 7,
		Table = {Standalone = true, Male = -1, Female = -1 },
...
function ResetClothing()
	...
	for k,v in pairs(LastEquipped) do
		if v then
			if v.Ped == Ped then
				if v.ID == 9 then 
					if lastArmor > 0 and GetPedArmour(PlayerPedId()) < lastArmor then 
...
local sencetiveEvents = {
	'skinchanger:loadSkin',
	'skinchanger:change',
	'skinchanger:loadClothes',
	'skinchanger:ChangePlayerPed',
}

--[[for k,v in pairs(sencetiveEvents) do
	RegisterNetEvent(v)
...
AddEventHandler("dpclothingAbuse", function(bool)
	Abuse = bool
end)
