...
local function CrouchKeyPressed()
    ...
    if configCrawl.CrouchOverride then
    ...
    CreateThread(function()
        ...
        while isProne do
            ...
            if not isCrawling then
                ...
                if forward or first then -- Forward
            ...
            if not isCrawling then
                ...
                if not IsEntityPlayingAnim(playerPed, "move_crawl", proneType.."_fwd", 3) then 
...
local function CrawlKeyPressed(dontPlayFirstAnim)
...
--     if configCrawl.CrouchKeybindEnabled then

--         RegisterKeyMapping('+crouch', configCrawl.Localization['crouch_keymapping'], "keyboard", configCrawl.CrouchKeybind)
...
--     TriggerEvent('chat:addSuggestion', '/crouch', configCrawl.Localization['crouch_chat_suggestion'])



--     if configCrawl.CrawlKeybindEnabled then

--         RegisterKeyMapping('+crawl', configCrawl.Localization['crawl_keymapping'], "keyboard", configCrawl.CrawlKeybind)
...
--     TriggerEvent('chat:addSuggestion', '/crouch', configCrawl.Localization['crawl_chat_suggestion'])
...
exports('toggleCrawl', CrawlKeyPressed)

exports("FlipCrawl", function()
    if proneType == "onfront" then
        CrawlFlip(PlayerPedId())
    end
end)