...
local REFRESH_RATE = math.max(100, math.min(500, ConfigOffRoad.refreshRate or 200))
                        ...
                        if wheelSize < 0.75 then
                            ...
                            if ConfigOffRoad.roadSideHelper.enabled then
                                ...
                                if nearestRoad <= ConfigOffRoad.roadSideHelper.distanceThreshold then
                                    realDepth = realDepth * ConfigOffRoad.roadSideHelper.depthMultiplier
                                end
                            end
                            ...
                            if sinkageSpeed > 0 then
                                sinkageSpeed = sinkageSpeed * (ConfigOffRoad.generalSinkageSpeed / 100)
                            end
...
function GetWheelSurface(veh, index)
    return UseCache('vehSurface_' .. veh .. '_' .. index, function()
        ...
        local surface = ConfigOffRoad.surfaces[surfaceId];
        if not surface then
            surface = ConfigOffRoad.fallbackSurface
        end
    ...
    while true do
        ...
        for i, veh in pairs(playerVehicles) do
            if DoesEntityExist(veh) then
                ...
                if didAnything then
                    sleep = math.floor(ConfigOffRoad.refreshRate * 2.5)
                end
            end
        end
            ...
            if GetWheelCount(veh) >= 3 then
                if wheelDepth ~= nil and wheelDepth > 0.01 and wheelDepth <= 1.5 then
                    if (EasyRound(wheelDepth) ~= EasyRound(GetVehicleWheelTireColliderSize(veh, index))) then
                        ...
                        if ConfigOffRoad.suspensionRefresh.enabled then
                            if ConfigOffRoad.suspensionRefresh.type == 'force' then
