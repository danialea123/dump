        ...
        if IsPedInAnyVehicle(playerPed, false) and GetPedInVehicleSeat(GetPlayerVeh(), -1) == playerPed then
            ...
            if CURRENT_DEPTH > 0.05 and not IsBlacklisted(veh) then
                ...
                if HasOffroadTires(veh) then
                    offroadTireUpgrade = ConfigOffRoad.offroadTires.upgradeValue
                end

                local totalUpgrades = generalUpgrade + turningUpgrade + offroadTireUpgrade

                if IsAwd(veh) then
                    totalUpgrades = (totalUpgrades + (ConfigOffRoad.awdUpgrade or 25)) * 1.3
                end

                local ConfigOffRoadMultiplier = ConfigOffRoad.generalDepthDifficulty / 100
                local totalSlowdown = math.floor(math.max(0, (math.floor(CURRENT_DEPTH * 820 * ConfigOffRoadMultiplier) * (math.max(0.9, CURRENT_SOFTNESS / 70))) - totalUpgrades))
...
if ConfigOffRoad.generalTractionLoss > 0 then
    Citizen.CreateThread(function()
        while true do
            local sleep = 1000
            local playerPed = PlayerPedId()
                ...
                if not IsBlacklisted(veh) and not IsInBlacklistedArea(veh) then
                    ...
                    if HasOffroadTires(veh) then
                        ...
                        if CURRENT_SOFTNESS < 10 then
                            traction = traction + ConfigOffRoad.offroadTires.tractionOnHard
                        else
                            traction = traction + ConfigOffRoad.offroadTires.tractionOnSoft
                        end
                    end

                    if ConfigOffRoad.roadSideHelper.enabled then
                        local nearestRoad = GetNearestRoadDistance()
                        if nearestRoad <= ConfigOffRoad.roadSideHelper.distanceThreshold then
                            traction = 100 - math.floor((100 - traction) * ConfigOffRoad.roadSideHelper.tractionMultiplier)
                        end
                    end
                    ...
                    if traction <= 95 then
                        ...
                        if drift > driftThreshold and (math.abs(GetVehicleThrottleOffset(veh)) > 0.1 or GetEntitySpeed(veh) > 5.0) and math.abs(GetVehicleSteeringAngle(veh)) > 0.2 then
                            ...
                            Citizen.Wait(200 - (CURRENT_TRACTION * (ConfigOffRoad.generalTractionLoss / 100)))
