	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				ESX.SetVehicleEngineHealth(vehicle, cfg.cascadingFailureThreshold + 5)
	...
	Citizen.CreateThread(function()
		while true do
			...
			if cfg.preventVehicleFlip then
				...
				if (roll > 75.0 or roll < -75.0) and GetEntitySpeed(vehicle) < 2 and not DecorGetBool(vehicle,"JobCenter") then
		...
		if isPedDrivingAVehicle() then
			...
			if healthEngineNew ~= healthEngineCurrent then
				if healthEngineNew < healthEngineCurrent then
					...
					ESX.SetVehicleEngineHealth(vehicle, healthEngineNew)
				end
			end
			if healthBodyNew ~= healthBodyCurrent then
				if healthBodyNew < healthBodyCurrent then
					...
					ESX.SetVehicleBodyHealth(vehicle, healthBodyNew)
				end
			end
