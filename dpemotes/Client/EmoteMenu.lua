      ...
      sharemenu.OnItemSelect = function(sender, item, index)
        if ShareTable[index] ~= 'none' then
          ...
          if(distance ~= -1 and distance < 3) then
            ...
            SimpleNotify(Config.Languages[lang]['sentrequestto']..GetPlayerServerId(target))
          else
...
function OpenEmoteMenu()
    if active and not IsPedInAnyVehicle(GetPlayerPed(-1),true) then
    ...
    --if not menuOpen then
      ...
      menuOpen = true
      tOut = ESX.SetTimeout(4 * 60 * 1000,function()
        menuOpen = false
      end)
      Citizen.CreateThread(function()
        while menuOpen do
