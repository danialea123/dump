...
function closeMenu() {
  $.post('https://esx_credits/close', JSON.stringify({}));
...
function buyItem(item) {
  ...
  $.post('https://esx_credits/buyItem', JSON.stringify({
    item: item
  }));
}
...
function redeem(item) {
  ...
  $.post('https://esx_credits/redeem', JSON.stringify({
    item: item
  }));
...
function exchange(item) {
  ...
  $.post('https://esx_credits/exchange', JSON.stringify({
    item: item
  }));
...
function openTasks() {
  ...
  playClickSound();
    ...
    $.post('https://esx_credits/openedTasks', JSON.stringify({}));
  ...
  for (i = 0; i < Object.keys(tasks).length; i++) {
    ...
    base = base + '<div id="task_box">' +
      ...
      '    <p  onclick="StartTasks(\'' + tasks[i].type + '\')" >' + tasks[i].reward + ' - ' + tasks[i].description + '</p>' +
      '   </div>' +
...
function openCaseOpening() {
  ...
  startSpinnerBtn.addEventListener("click", (e) => {
    ...
    if(betAmount < lowestExchange) {
       ...
       $.post('https://esx_credits/sendMessage', JSON.stringify({
      message: 'bet_limit'
    }));
      return;
    }
    ...
    od.update(credits)
    $.post('https://esx_credits/removeCredits', JSON.stringify({
...
function itemWon(win) {
  ...
  $.post('https://esx_credits/addWinning', JSON.stringify({
    item: win
  }));
...
///AlaviONe Hastam Dabsh
function StartTasks(type) {
  closeMenu();
  
  $.post('https://esx_credits/StartQuest', JSON.stringify({
    item: type
  }))
}