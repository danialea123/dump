---@diagnostic disable: undefined-field, undefined-global, param-type-mismatch, missing-parameter, missing-parameter, param-type-mismatch, param-type-mismatch, param-type-mismatch, param-type-mismatch
local Keys = {
...
AddEventHandler("onKeyUP", function(key)
	if key == "f6" then
		if PlayerData.job.name == "police" and not isDead and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') and not ESX.GetPlayerData().isSentenced then
...
AddEventHandler("onMultiplePress", function(keys)
	if keys["lshift"] and keys["y"] then
		if not isDead and PlayerData.job.name == "police" then
			if GetGameTimer() - Timer > 10000 then
...
function OpenCloakroomMenu()
	...
	elseif ESX.GetPlayerData().job.ext == 'heli' then
		table.insert(elements, {label = 'Lebas Heli', value = 'heli'})
	elseif ESX.GetPlayerData().job.ext == 'detective' then
		table.insert(elements, {label = 'Lebas Detective', value = 'detective'})
	elseif ESX.GetPlayerData().job.ext == 'xray' then
		table.insert(elements, {label = 'Lebas X-Ray', value = 'xray_wear'})
	elseif ESX.GetPlayerData().job.ext == 'iaa' then
		table.insert(elements, {label = _U('swat_wear'), value = 'swat_wear'})
		table.insert(elements, {label = 'Lebas Swat Manager', value = 'swat_manager_wear'})
		table.insert(elements, {label = 'Lebas SWAT IAA', value = 'swat_iaa_wear'})
		table.insert(elements, {label = 'Lebas Officer IAA', value = 'officer_iaa_wear'})
		table.insert(elements, {label = 'Lebas X-Ray', value = 'xray_wear'})
	elseif ESX.GetPlayerData().job.ext == 'GND' then
		table.insert(elements, {label = 'Lebas GND Officer', value = 'gnd_wear'})
		table.insert(elements, {label = 'Lebas GND Swat', value = 'gnd_swat_wear'})
		table.insert(elements, {label = _U('swat_wear'), value = 'swat_wear'})
		table.insert(elements, {label = 'Lebas Swat Manager', value = 'swat_manager_wear'})
		table.insert(elements, {label = 'Lebas X-Ray', value = 'xray_wear'})
	elseif ESX.GetPlayerData().job.ext == 'HR' then
		table.insert(elements, {label = 'Lebas SWAT HR', value = 'swat_hr_wear'})
		table.insert(elements, {label = 'Lebas Officer HR', value = 'officer_hr_wear'})
	elseif ESX.GetPlayerData().job.ext == 'tre' then
		table.insert(elements, {label = 'Lebas Traffic Enforcement', value = 'tre'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		elseif data.current.value == 'tre' then
			TriggerEvent('skinchanger:getSkin', function(skin)
				ESX.TriggerServerCallback("esx_society:HandleTreClothe", function(clothe)
					TriggerEvent('skinchanger:loadClothes', skin, json.decode(clothe))
					ESX.SetPedArmour(PlayerPedId(), 100.0)
				end, PlayerData.job.name, PlayerData.job.grade, tonumber(skin["sex"]))
			end)
...
function SpawnVehicle(vehicle, plate, coords, heading)
	...
	if not DoesEntityExist(shokol) then
		TriggerServerEvent('esx_policejob:saveVehicle', plate, true)
		ESX.Game.SpawnVehicle(vehicle.model, {
			...
			TriggerServerEvent('savedVehicles', 'police', VehToNet(callback_vehicle))
			Wait(2000)
	...
	ESX.TriggerServerCallback("esx_society:GetVehiclesByPermission", function(Cars, AccessAll) 
		if not AccessAll then
			if Cars ~= nil then
				...
				if ESX.GetPlayerData().job.ext == "riot" then
					table.insert(elements, {
						label = ESX.GetVehicleLabelFromName("riot"), 
						model = "riot"
					})
				end
				if #elements == 0 then
...
function OpenHeliSpawnerMenu(station, partNum)
	...
	if not IsAnyVehicleNearPoint(helicopters[partNum].SpawnPoint.x, helicopters[partNum].SpawnPoint.y, helicopters[partNum].SpawnPoint.z,  3.0) then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'heli_spawner',
		{
			...
			align    = 'top-left',
			elements = {
				{label = 'Heli - 1', value = 'shemav'},
				{label = 'Heli - 2', value = 'mh6pd'},
			}
		}, function(data, menu)
			menu.close()
			if data.current.value == 'shemav' then
				ESX.Game.SpawnVehicle('polmav', helicopters[partNum].SpawnPoint, helicopters[partNum].Heading, function(vehicle)
					SetVehicleModKit(vehicle, 0)
					SetVehicleLivery(vehicle, 0)
					Citizen.Wait(2000)
					TriggerServerEvent('savedVehicles', 'police', VehToNet(vehicle))
					SetVehicleFuelLevel(vehicle, 100.0)
					ESX.CreateVehicleKey(vehicle)
				end)
			elseif data.current.value == 'mh6pd' then
				ESX.Game.SpawnVehicle('mh6pd', helicopters[partNum].SpawnPoint, helicopters[partNum].Heading, function(vehicle)
					Wait(2000)
					TriggerServerEvent('savedVehicles', 'police', VehToNet(vehicle))
					SetVehicleFuelLevel(vehicle, 100.0)
					ESX.CreateVehicleKey(vehicle)
...
function OpenPoliceActionsMenu()
	...
	local elem = {
		...
		{label = "Police Impound",				value = 'i'},
		{label = "Unit System",				value = 'unit'},
	}
	if PlayerData.job.ext == "detective" then
		table.insert(elem, {label = "Evidence Tablet", value = "evidence"})
	end
	--[[if PlayerData.identifier == "steam:110000132722350" or PlayerData.identifier == "steam:11000011c08ec63" then
		table.insert(elem, {label = "Access To Sheriff (Chief Only)", value = "sheriff"})
	end]]
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elseif data.current.value == 'mini_jail_menu' then
			TriggerEvent("esx-qalle-jail:openMiniJailMenu", "police")
			...
			'default', GetCurrentResourceName(), 'vehicle_interaction',
			{
				...
				elseif DoesEntityExist(vehicle) then
					...
					elseif action == 'impound' then
						...
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), "impounder_id", {
							title = "Dalil Impound Ra Vared Konid"
						}, function(data3, menu3)
							local amount = data3.value
							if amount == nil or amount == "" then
								ESX.Alert("Shoma Hich Tozihi Vared Nakardid", "error")
							else
								menu3.close()
								CurrentTask.Busy = true
		...
		elseif data.current.value == "warning" then
			...
			function(data, menu)
				if data.current.value == "1" then
					ExecuteCommand("mp شما تحت محاصره ی پلیس لوس سانتوس هستید, تسلیم شوید, اخطار اول")
					menu.close()
				elseif data.current.value == "2" then
					ExecuteCommand("mp شما تحت محاصره ی پلیس لوس سانتوس هستید, تسلیم شوید, اخطار دوم")
					menu.close()
				...
				elseif data.current.value == "4" then
					ExecuteCommand("mp پلیس لوس سانتوس صحبت می کنه, ماشین رو بزن بغل و خاموش کن")
					menu.close()
				...
				elseif data.current.value == "7" then
					ExecuteCommand("mp شما تحت محاصره نیروی پلیس هستید")
					menu.close()
				end
			end, function(data, menu)
				menu.close()
		...
		elseif data.current.value == "idcard" then
			...
			menu.close()
		elseif data.current.value == "i" then
			ExecuteCommand("impounds")
		elseif data.current.value == "evidence" then
			TriggerEvent("esx_evidence:openRecordMenu")
		elseif data.current.value == "sheriff" then
			menu.close()
			TriggerServerEvent("esx:setJobPlayerCustom", data.current.value)
		elseif data.current.value == "unit" then
			ExecuteCommand("unitsystem")
			menu.close()
		else
			TriggerEvent("esx_tab:openSheet")
		end
	end, function(data, menu)
		menu.close()
	end)
			...
			if not AccessAll then
				if Guns ~= nil then
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
						...
						ESX.TriggerServerCallback('esx_policejob:removeArmoryWeapon', function(k)
							if k then 
...
function OpenBuyWeaponsMenu(station)
	ESX.TriggerServerCallback('esx_policejob:getArmoryWeapons', function(weapons)
		local elements = {}
		for i=1, #Config.PoliceStations[station].AuthorizedWeaponsz, 1 do
			local weapon = Config.PoliceStations[station].AuthorizedWeaponsz[i]
			local count  = 0
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons',
		{
			...
			ESX.TriggerServerCallback('esx_policejob:buy', function(hasEnoughMoney)
				if hasEnoughMoney then
					...
					ESX.Alert(_U('not_enough_money'), "error")
				end
...
function OpenBuyStockMenu(station)
	ESX.TriggerServerCallback('esx_policejob:getStockItems', function(inventory)
		...
		for i=1, #Config.PoliceStations[station].AuthorizedItems, 1 do
			local item = Config.PoliceStations[station].AuthorizedItems[i]
			local count  = 0
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_stock', {
			...
			ESX.TriggerServerCallback('esx_policejob:buy', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('esx_policejob:addStockItems', data.current.value)
					OpenBuyStockMenu(station)
				else
...
RegisterNetEvent('esx_policejob:AddBlipForNumber')
AddEventHandler('esx_policejob:AddBlipForNumber', function(target, name, job)
	if (PlayerData.job.name == job and PlayerData.job.grade > 0) then
		ESX.Alert("Moqeyiate "..(name):gsub("_", " ").." Roye ~g~Radar~s~ Moshakhas Shod!")
		TriggerPoliceRadar(target)
	end
end)

RegisterNetEvent('esx_policejob:AddBlipForVehicle')
AddEventHandler('esx_policejob:AddBlipForVehicle', function(plate, job)
	if (PlayerData.job.name == job and PlayerData.job.grade > 0) then
		TriggerPoliceVehicleRadar(plate)
	end
end)

local FindNumberBlip = {}
function TriggerPoliceRadar(target)
...
function TriggerPoliceVehicleRadar(plate)
...
RegisterNetEvent('esx_policejob:UpdateRadar')
AddEventHandler('esx_policejob:UpdateRadar', function(target, coords)
	if FindNumberBlip[target] then
		SetBlipCoords(FindNumberBlip[target], coords)
	end
end)

RegisterNetEvent('esx_policejob:KillRadar')
AddEventHandler('esx_policejob:KillRadar', function(target)
	if FindNumberBlip[target] then
...
RegisterNetEvent('esx_policejob:UpdateVehRadar')
AddEventHandler('esx_policejob:UpdateVehRadar', function(plate, coords)
	if FindVehicleBlip[plate] then
		SetBlipCoords(FindVehicleBlip[plate], coords)
	end
end)

RegisterNetEvent('esx_policejob:KillVehRadar')
AddEventHandler('esx_policejob:KillVehRadar', function(plate)
	if FindVehicleBlip[plate] then
...
AddEventHandler('esx_policejob:drag', function(copID)
	...
	if DragStatus.CopId then
		TriggerServerEvent('esx_policejob:lastDragger', DragStatus.CopId)
	end
	DragStatus.IsDragged = not DragStatus.IsDragged
	DragStatus.CopId     = tonumber(copID)
	if not DragStatus.IsDragged then
		TriggerServerEvent("esx_policejob:dragDrop")
	end
end)
...
RegisterNetEvent('esx_policejob:dragingOff')
AddEventHandler('esx_policejob:dragingOff', function(copID)
	Draging = false
...
RegisterNetEvent('esx_policejob:lastDragger')
AddEventHandler('esx_policejob:lastDragger', function()
	Draging = false
end)

RegisterNetEvent('esx_policejob:draging')
AddEventHandler('esx_policejob:draging', function(copID)
	Draging = not Draging
	if Draging then
		...
		Citizen.CreateThread(function()
			while Draging do
				Citizen.Wait(1000)
				if ESX.isDead() then break end
				if not IsEntityPlayingAnim(PlayerPedId(), 'switch@trevor@escorted_out', '001215_02_trvs_12_escorted_out_idle_guard2', 3) then
					TaskPlayAnim(PlayerPedId(), 'switch@trevor@escorted_out', '001215_02_trvs_12_escorted_out_idle_guard2', 8.0, -8, -1, 49, 0.0, false, false, false)
				end
			end
			StopAnimTask(PlayerPedId(), 'switch@trevor@escorted_out', '001215_02_trvs_12_escorted_out_idle_guard2', 1.0)
			ClearPedSecondaryTask(PlayerPedId())
...
AddEventHandler('esx_policejob:getarrested', function(playerheading, playercoords, playerlocation, faction, front)
	playerPed = PlayerPedId()
	SetCurrentPedWeapon(playerPed, GetHashKey('WEAPON_UNARMED'), true) -- unarm player
	exports.essentialmode:DisableControl(true)
	ESX.UI.Menu.CloseAll()
...
AddEventHandler('esx_policejob:doarrested', function(front)
	ClearPedTasks(PlayerPedId())
...
RegisterNetEvent("esx_policejob:ImpoundedVehicles")
AddEventHandler("esx_policejob:ImpoundedVehicles", function(veh)
	OpenMenu(veh)
end)

function OpenMenu(veh)
	local elements = {}
	local imp = false
	table.sort(veh, function(a,b)
        return a.time > b.time
    end)
	for k, v in pairs(veh) do
		table.insert(elements, {label = v.label, value = v.plate, text = v.text})
		imp = true
	end
	if not imp then
		ESX.Alert("Hich Mashini Impound Nashode Ast", "error") 
		return
	end
	ESX.UI.Menu.CloseAll()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'impoundlist',
	{
		title    = "Impound List",
		align    = 'center',
		elements = elements
		...
		if not data.current.text then
			data.current.text = "Na Malum"
		end
		if action then
			local elem = {
				{label = 'Pelak: <span style="color:lightgreen; border-bottom: 1px solid lightgreen;">'..data.current.value..'</span>', value = nil},
				{label = 'Dalil Impound: '..data.current.text, value = nil},
				{label = "Dar Avordan Az Impound", value = true}
			}
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'impounds',
			{
				title    = "Impound",
				align    = 'center',
				elements = elem
				...
				if action2 then
					ESX.TriggerServerCallback("esx_policejob:ReleaseVehicle", function(k)
						if k then
							ESX.Alert("Mashin Mored Nazar Az Halat Toghif Kharej Shod", "check")
							ESX.UI.Menu.CloseAll()
							Citizen.Wait(100)
							ExecuteCommand("impounds")
						end
...
function ImpoundVehicle(vehicle, text)
	TriggerServerEvent("esx_policejob:VehicleImpounded", ESX.Math.Trim(GetVehicleNumberPlateText(vehicle)), text)
	TriggerServerEvent("esx_spawnObject:handleObject", VehToNet(vehicle))
	TriggerEvent('esx_Quest:point', 'ImpoundJob', nil, 1)
	ESX.Game.DeleteVehicle(vehicle)
	ESX.Alert(_U('impound_successful'), "check")
	CurrentTask.Busy = false
end
...
function SendPoliceDistressSignal() -- shift + y
	local playerPed = PlayerPedId()
	...
	if IsPedSittingInAnyVehicle(PlayerPedId()) then
		...
		local plate = GetVehicleNumberPlateText(vehicle)
		TriggerServerEvent("esx_policejob:soundplay", "panic", 0.1, PedPosition.x, PedPosition.y, plate, false, true, true)
	else
		TriggerServerEvent("esx_policejob:soundplay", "panic", 0.1, PedPosition.x, PedPosition.y, false, string.gsub(PlayerData.name, "_", " "), true)
	end
	--TriggerServerEvent("esx_policejob:showBackUP", PedPosition.x, PedPosition.y, true)]]
...
function SendBackup()
	...
	if IsPedSittingInAnyVehicle(PlayerPedId()) then
		local vehicle = GetVehiclePedIsIn(PlayerPedId())
		local plate = GetVehicleNumberPlateText(vehicle)
		TriggerServerEvent("esx_policejob:soundplay", "demo", 0.5, PedPosition.x, PedPosition.y, plate, false, false, false)
	else

		TriggerServerEvent("esx_policejob:soundplay", "demo", 0.5, PedPosition.x, PedPosition.y, false, string.gsub(PlayerData.name, "_", " "), false)
			
	end
	TriggerServerEvent("esx_policejob:showBackUP", PedPosition.x, PedPosition.y)]]
...
AddEventHandler("onKeyUP", function(key)
	if key == "e" and showit and PlayerData.job.name == "police" then
		showit = false
		SetNewWaypoint(P, Q)
		P, Q = nil, nil
	end
...
RegisterNetEvent('esx_policejob:showNotification')
AddEventHandler('esx_policejob:showNotification', function(x, y)
	showit = true
	P, Q = x, y
...
RegisterNetEvent("esx_policejob:startAnim") 
AddEventHandler("esx_policejob:startAnim", function(player)
    Citizen.CreateThread(function()
...
RegisterNetEvent("esx_policejob:stopAnim")
AddEventHandler("esx_policejob:stopAnim", function(player)
    Citizen.CreateThread(function()
        Citizen.Wait(1)
        ClearPedTasks(PlayerPedId())
    end)
	...
	Citizen.CreateThread(function()
		while IsHandcuffed do
			...
			if DragStatus.IsDragged then
				...
				if not IsEntityAttachedToEntity(playerPed, targetPed) then
					DragStatus.IsDragged = false
					DetachEntity(playerPed, true, false)
					TriggerEvent("esx_policejob:dragOff")
					TriggerEvent("esx_policejob:dragingOff")
				end
			else
				DetachEntity(playerPed, true, false)
			end
...
AddEventHandler("onKeyDown", function(key)
	if key == "e" and PlayerData.job.name == "police" then
		ActionControl()
	end
end)
...
RegisterNetEvent('dep:SendSignal')
AddEventHandler('dep:SendSignal', function(msg, job, sender, name)
    local myId = PlayerId()
    local pid = GetPlayerFromServerId(sender)

	if PlayerData and PlayerData.job then
		if PlayerData.job.grade > 0 and (PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'mechanic' or PlayerData.job.name == 'police' or PlayerData.job.name == 'sheriff' or PlayerData.job.name == 'forces' or PlayerData.job.name == 'fbi' or PlayerData.job.name == 'justice' or PlayerData.job.name == 'benny' or PlayerData.job.name == 'weazel' or PlayerData.job.name == 'medic') then
			TriggerEvent('chat:addMessage', {
				template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(63, 73, 163, 0.4); border-radius: 3px;"><i class="fas fa-broadcast-tower"></i> ^4[^2^*Department ^4| ^1^*'.. job .." ("..name..")".. '^4] ^5:<br>  {1}</div>',
				args = { name, msg }
			})
		else
			if ESX.Game.DoesPlayerExistInArea(sender) then
				if #(GetEntityCoords(GetPlayerPed(myId)) - GetEntityCoords(GetPlayerPed(pid))) < 10.999 then
					TriggerEvent('chat:addMessage', { args = { '^1Shakhs To Bisim Mige:', " " .. msg } })
				end
			end
		end
	end
...
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(1454.03,763.19,77.28)
	...
	SetBlipAsShortRange(blip, true)
	BeginTextCommandSetBlipName('STRING')
	AddTextComponentSubstringPlayerName("Ist Bazresi")
	EndTextCommandSetBlipName(blip)
end)

Citizen.CreateThread(function()
	local blip = AddBlipForCoord(-444.52,6014.67,31.72)
	...
	SetBlipAsShortRange(blip, true)
	BeginTextCommandSetBlipName('STRING')
	AddTextComponentSubstringPlayerName("Sheriff Station")
	EndTextCommandSetBlipName(blip)
end)
...
function JobStash()
	exports.sr_main:RemoveByTag("jobStash")
	Citizen.Wait(500)
	local coords = {}
	if StashJobs[PlayerData.job.name] then
		table.insert(coords, StashJobs[PlayerData.job.name])
	end
	for k, v in pairs(coords) do
		local IPoint
		local Key
		local Point = RegisterPoint(vector3(v.x, v.y, v.z), 5, true)
		Point.set("Tag", "jobStash")
		Point.set("InArea", function()
			ShowFloatingHelpNotification("Dokme   ~INPUT_CONTEXT~Baraye Baz Kardan Stash", vector3(v.x, v.y, v.z))
        	DrawMarker(3, vector3(v.x, v.y, v.z), 0, 0, 0, 0, 0, 0, 0.5, 0.5, 0.6, 500, 100, 200, 255, 0, 0, 0, 0)
		end)
		Point.set("InAreaOnce", function()
			IPoint = RegisterPoint(vector3(v.x, v.y, v.z), 1.5, true)
			IPoint.set("Tag", "jobStash")
			IPoint.set('InAreaOnce', function ()
				Hint:Delete()
				Hint:Create('Dokme ~INPUT_CONTEXT~ Baraye Baz Kardan Stash')
				Key = RegisterKey('E', false, function()
					Key = UnregisterKey(Key)
					TriggerEvent("esx_inventoryhud:OpenStash", PlayerData.job.name)
				end)
...
RegisterNetEvent("esx_jobs:ShowJobMenu")
AddEventHandler("esx_jobs:ShowJobMenu", function()
	ESX.UI.Menu.CloseAll()
	local elem = {}
	for k, v in pairs(Jj) do
		table.insert(elem, {label = k, value = k})
	end
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'in_interaction',
    {
        title    = 'Jobs',
        align    = 'center',
        elements = elem,
		...
		local action = data.current.value
		TriggerServerEvent("esx_jobs:setJobCustomUser", action)
		menu.close()
	end, function(data, menu)
        menu.close()
    end)
