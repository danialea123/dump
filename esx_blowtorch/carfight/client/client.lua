...
function GetPlayerActiveArena()
    for k, v in pairs(Config.DerbyArenaNameList) do
        if esx_sit:IsPlayerInArena(v) then
            return v
        end
    end
    return "none"
...
for k, v in pairs(Config.DerbyArenaNameList) do
    esx_sit:OnPlayerExitLobby(v, function()
        SetDisplay(false)
        ...
        InCar = false
        if ArenaBusy then
            ESX.SetEntityCoords(PlayerPedId(), PrevPos.x, PrevPos.y, PrevPos.z)
            vehicle = nil
            ArenaBusy = false
        end
    end)
...
CreateThread(function()
    while true do
        Wait(15000)
        if esx_sit:IsPlayerInArena(GetPlayerActiveArena()) and ArenaBusy then
            local playerVehicle = GetVehiclePedIsIn(PlayerPedId(), false)
            SetVehicleEngineHealth(playerVehicle, 1000)
            SetVehicleEngineOn(playerVehicle, true, true)
            ESX.SetVehicleFixed(playerVehicle)
        end
    end
end)
...
CreateThread(function()
    while true do
        Wait(1000)
        if esx_sit:IsPlayerInArena(GetPlayerActiveArena()) and ArenaBusy and vehicle ~= nil then
...
CreateThread(function()
    while true do
        Citizen.Wait(500)
        if esx_sit:IsPlayerInArena(GetPlayerActiveArena()) and ArenaBusy and InCar then
            if GetPlayerActiveArena() ~= "none" and esx_sit:GetArenaLabel(GetPlayerActiveArena()) then
                local coord
                local dis
                if esx_sit:GetArenaLabel(GetPlayerActiveArena()) == "derby" then
                    coord = Config.ArenaLocation
                    dis = Config.MinimumZLevel
                else
                    coord = Config.ArenaLocation2
                    dis = Config.MinimumZLevel2
                end
                if #(GetEntityCoords(PlayerPedId()) - coord) > Config.ArenaDistance or GetEntityCoords(PlayerPedId()).z < dis then
                    ...
                    ESX.SetEntityCoords(playerVehicle, position.x, position.y, position.z)
...
AddEventHandler("rcore_derby:ArenaEnded", function(sorted)
    ...
    local func = {
        [1] = function()
            ...
            ESX.SetEntityCoords(ped, v.Position.x, v.Position.y, v.Position.z)
    ...
    ESX.SetEntityCoords(PlayerPedId(), vv.Position.x, vv.Position.y, vv.Position.z)
    ...
    for k, v in pairs(sorted) do
        ...
        if v.id == ServerID then
            if func[k] then
                ...
                ESX.SetEntityCoords(ped, v.x, v.y + minus, v.z)
...
AddEventHandler("rcore_derby:ArenaStarted", function(data)
    ...
    ESX.SetEntityCoords(playerPed, coords.x, coords.y, coords.z)
    Wait(1000)
