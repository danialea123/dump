...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for i, self in pairs(nearMarkersV2) do
            if self.onlyVehicle == nil or IsPedAnyVehicle(getPed(), false) == self.onlyVehicle then
                local distance = #(coords - self.position)
...
function createMarker(res)
    ...
    ------------------------------
    self.setOnlyVehicle = function(result)
        self.onlyVehicle = result and 1 or false
    end
    ------------------------------
