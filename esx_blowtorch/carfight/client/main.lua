...
CreateThread(function()
    Wait(1000)
    for k, v in pairs(Cfg.MarkerList) do
        ...
        marker.setKeys(Cfg.KeyListToInteract)
        ...
        marker.render()
        local arena = exports.esx_sit:GetArena(k)
        if arena then
...
AddEventHandler("ArenaAPI:sendStatus", function(type, data)
    -- just to make sure the data will get updated first in "esx_sit" i will make a dedicated event later for this.
    Wait(1000)
    if Cached3DText[data.ArenaIdentifier] then
        Cached3DText[data.ArenaIdentifier].setText(string.format(Cfg.MarkerList[data.ArenaIdentifier].text, data.CurrentCapacity, data.MaximumCapacity))
    end
end)