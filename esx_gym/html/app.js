...
window.addEventListener("load", function(){
	...
	$.post(`https://esx_gym/action`, JSON.stringify({action: "load", notifyStatus: localStorage.getItem("notify-status")}));
});
...
$('.status-button').click(function() {
	...
	$.post(`https://esx_gym/action`, JSON.stringify({action: "notifyStatus", status: localStorage.getItem("notify-status")}));
})
...
document.onkeyup = function(data) {
	if (data.which == 27) {
		$.post(`https://esx_gym/action`, JSON.stringify({action: "closeMenu"}));
	}
};