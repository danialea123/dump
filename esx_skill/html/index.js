...
function closeMenu() {
  $.post("https://esx_skill/close", JSON.stringify({}));
  playClickSound();
...
function mainMenu() {
  var base =
    ...
    "</span>" +
    '<span class="e1_11">JOB :<span class="regular"> ' +
    job.toUpperCase() +
    "</span></span>" +
    '<span class="e1_12">LEVEL :<span class="regular1"> ' +
    level +
    "</span></span>" +
    "</div>" +
    "<div class=e7_50>" +
    ...
    "</div>" +
    '<span class="e7_41">' +
    Config.StartingSkill.SkillName +
    " " +
    firstname +
    "</span>" +
    '<div class="e7_43"><span id="text">' +
    Config.StartingSkill.SkillDescription +
    "</span></div>";

  if (ownedskills.includes("StartingSkill") == false) {
    base =
      base +
      '<span class="e7_105">PURCHASE (' +
      Config.StartingSkill.Purchase +
      ")</span>";
  } else {
    base =
      base +
  ...
  if (ownedskills.includes("StartingSkill") == true) {
    base =
      ...
      '           <div id="skillpointimg" style="background-image: url(img/skillpoints/' +
      Config.StartingSkill.image +
      '.png);"></div>' +
      "       </div>" +
      "      </div>" +
      "    </div>" +
  ...
  for (const [key3, value3] of Object.entries(Config.Skills)) {
    ...
    if (ownedskills.includes(key3)) {
      base =
        base +
        '<div class="smallbackgroundreactor" style="left:' +
        x +
        ";top:" +
        y +
        ';">' +
  ...
  $(document).ready(function () {
    $(".core").hover(function () {
      ...
      if (ownedskills.includes($(this).attr("Skill"))) {
        ...
        $(".e7_105").text(
          "REQUERES " + $(this).attr("Purchase") + " SKILL POINTS"
        );
        ...
        if (ownedskills.includes(value3.ConnectLineTo) == true) {
          if (key3 == $(this).attr("Skill")) {
            if (ownedskills.includes($(this).attr("Skill")) == false) {
              ...
              $("#questionPurchase").text(
                "COST " + $(this).attr("Purchase") + " SKILL POINTS"
              );
          ...
          setTimeout(function () {
            ...
            setTimeout(function () {
              ...
              setTimeout(function () {
                currentBubble
                  .parent(".smallcase_container")
                  .parent(".smallbackgroundreactor")
                  .addClass("smallarc_reactor");
              }, 200);
            }, 600);
          }, 200);
        }, 300);
  ...
  $(document).ready(function () {
    $("#scalingdiv").draggable({
      ...
      drag: function (event, ui) {
        ...
        if (
          ui.position.left <
          -(width - screenwidth) + (width * (1 - __scale)) / 2
        ) {
          ui.position.left =
            -(width - screenwidth) + (width * (1 - __scale)) / 2;
        }
        ...
        if (
          ui.position.top <
          -(height - screenheight) + (height * (1 - __scale)) / 2
        ) {
          ui.position.top =
            -(height - screenheight) + (height * (1 - __scale)) / 2;
        }
...
function buySkill(Skill, Purchase) {
  $.post(
    "https://esx_skill/buySkill",
    JSON.stringify({ Skill: Skill, Purchase: Purchase })
  );
  playClickSound();
}
...
function StartingSkill(StartingSkillPoints, StartingSkillId) {
  $.post(
    "https://esx_skill/StartingSkill",
    JSON.stringify({
      StartingSkillPoints: StartingSkillPoints,
...
function RestoreDefault() {
  $.post("https://esx_skill/RestoreDefault", JSON.stringify({}));
}
  ...
  for (const [key3, value3] of Object.entries(Config.Skills)) {
    if (currentBubble.attr("Skill") == key3) {
      $(".line").each(function (index, value) {
        ...
        if (
          LineStart.includes(key3) &&
          LineEnd.includes(value3.ConnectLineTo)
        ) {
          $(this).css(
            "background",
            "linear-gradient(to left, rgba(2, 255, 255, 0.5) 40%, rgba(255,255,255,1) 50%) right"
          );
...
function playNotificationSound() {
  ...
  var isPlaying =
    audio.currentTime > 0 && !audio.paused && !audio.ended && audio.readyState;
...
function NotificationLevelUp(addedpoints, experience) {
  ...
  console.log(addedpoints, experience);
  var base = '<div id="notifbackground">' + '<div id="notifdiv">';
  base =
    base +
    ...
    level +
    "</div>" +
    '<div id="notifSkillPoint">+' +
    addedpoints +
    " SKILL POINT</div>" +
    '<div id="notifExperience">+' +
    experience +
    " EXPERIENCE</div>" +
...
function formatTime(time) {
  /// jeigu reik kad rasytu virusi naudoju
      ...
      if (key3 == value4.ConnectLineTo) {
        ...
        if (ownedskills.includes(lineStart) && ownedskills.includes(lineEnd)) {
          htmlLine =
            ...
            lineEnd +
            '"' +
            'style="height:8px; background:linear-gradient(90deg, rgba(2, 255, 255, 0.5) 70%, rgba(255,255,255,1) 100%); line-height:1px; position:absolute; left:' +
            cx +
            "px; top:" +
            cy +
            "px; width:" +
          ...
          htmlLine =
            ...
            lineEnd +
            '"' +
            'style="height:8px; background-color:white; line-height:1px; position:absolute; left:' +
            cx +
            "px; top:" +
            cy +
            "px; width:" +
        ...
        if (ResetAvailable == true) {
          for (const [key3, value3] of Object.entries(Config.Skills)) {
            if (ownedskills.includes(key3) == true) {
              ...
              $(".core2").css(
                "box-shadow",
                "0px 0px 0px rgba(2, 254, 255, 0.8)"
              );
