...
AddEventHandler("onKeyUP", function(key)
	if key == "f6" then
		if PlayerData.job.name == "sheriff" and not isDead and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'sheriff_actions') and not ESX.GetPlayerData().isSentenced then
...
AddEventHandler("onMultiplePress", function(keys)
	if keys["lshift"] and keys["y"] then
		if not isDead and PlayerData.job.name == "sheriff" then
			if GetGameTimer() - Timer > 10000 then
				SendsheriffDistressSignal()
				Timer = GetGameTimer()
			else
				ESX.Alert("Spam Nakonid", "error")
			end
...
function SpawnVehicle(vehicle, plate, coords, heading)
	...
	if not DoesEntityExist(shokol) then
		TriggerServerEvent('esx_sheriffjob:saveVehicle', plate, true)
		ESX.Game.SpawnVehicle(vehicle.model, {
			...
			TriggerServerEvent('savedVehicles', 'sheriff', VehToNet(callback_vehicle))
			Wait(2000)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'car_plate', {
				...
				if data3.value == nil then
					...
					print()
					ESX.TriggerServerCallback('esx:GetVehicleByPlate', function(veh)
						ESX.TriggerServerCallback('esx_sheriffjob:callbackVehicle', function(status)
							if status then
					ESX.Alert('In Pelak Ghablan Vared Shode Va Dar Hal Hazer Motalegh Be Yek Nafar Mibashad!', "info")
							else
...
function OpenHeliSpawnerMenu(station, partNum)
	local helicopters = Config.SheriffStations[station].Helicopters
	ESX.UI.Menu.CloseAll()
...
function OpensheriffActionsMenu()
	...
	--[[if PlayerData.identifier == "steam:110000132722350" or PlayerData.identifier == "steam:11000011c08ec63" then
		table.insert(elem, {label = "Access To Police (Chief Only)", value = "police"})
	end]]
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sheriff_actions', {
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'give_weap_license' then
						ChooseFromNearbyPlayers(2, function(target)
							TriggerServerEvent('esx_sheriffjob:GiveWeaponLicese', target)
						end)
		...
		elseif data.current.value == 'mini_jail_menu' then
			TriggerEvent("esx-qalle-jail:openMiniJailMenu", "sheriff")
		...
		elseif data.current.value == "warning" then
			...
			function(data, menu)
				...
				elseif data.current.value == "7" then
					ExecuteCommand("mp شما تحت محاصره نیروی شریف هستید")
					menu.close()
				end
			end, function(data, menu)
				menu.close()
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'custom_fine_number', {
				...
				if amount ~= nil then
					if amount <= 100000 then
						menu2.close()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_sheriff', _U('fine_total', reason), amount, true)
						TriggerServerEvent('DiscordBot:ToDiscord', 'jobsfine', 'JOBS FINE LOG', '***sheriff Fine***\n```css\n[FROM]: ('..GetPlayerServerId(PlayerId())..') '..PlayerData.name..'\n[TO]: ('..GetPlayerServerId(player)..') '..GetPlayerName(player)..'\n[AMOUNT]: '..ESX.Math.GroupDigits(amount)..'\n[REASON]: '..reason..'```', 'user', GetPlayerServerId(PlayerId()), true, false)
					else
			...
			if not AccessAll then
				if Guns ~= nil then
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
						...
						ESX.TriggerServerCallback('esx_sheriffjob:removeArmoryWeapon', function(k)
							if k then 
...
function OpenBuyStockMenu(station)
	ESX.TriggerServerCallback('esx_sheriffjob:getStockItems', function(inventory)
		local elements = {}

		for i=1, #Config.SheriffStations[station].AuthorizedItems, 1 do
			local item = Config.SheriffStations[station].AuthorizedItems[i]
			local count  = 0
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_stock', {
			...
			ESX.TriggerServerCallback('esx_sheriffjob:buy', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('esx_sheriffjob:addStockItems', data.current.value)
					OpenBuyStockMenu(station)
				else
					ESX.Alert(_U('not_enough_money'))
				end
...
RegisterNetEvent('esx_sheriffjob:AddBlipForNumber')
AddEventHandler('esx_sheriffjob:AddBlipForNumber', function(target, name, job)
	if (PlayerData.job.name == job and PlayerData.job.grade > 0) then
		ESX.Alert("Moqeyiate "..(name):gsub("_", " ").." Roye ~g~Radar~s~ Moshakhas Shod!")
		TriggersheriffRadar(target)
	end
end)

RegisterNetEvent('esx_sheriffjob:AddBlipForVehicle')
AddEventHandler('esx_sheriffjob:AddBlipForVehicle', function(plate, job)
	if (PlayerData.job.name == job and PlayerData.job.grade > 0) then
		TriggersheriffVehicleRadar(plate)
	end
end)

local FindNumberBlip = {}
function TriggersheriffRadar(target)
...
function TriggersheriffVehicleRadar(plate)
...
RegisterNetEvent('esx_sheriffjob:UpdateRadar')
AddEventHandler('esx_sheriffjob:UpdateRadar', function(target, coords)
	if FindNumberBlip[target] then
		SetBlipCoords(FindNumberBlip[target], coords)
	end
end)

RegisterNetEvent('esx_sheriffjob:KillRadar')
AddEventHandler('esx_sheriffjob:KillRadar', function(target)
	if FindNumberBlip[target] then
...
RegisterNetEvent('esx_sheriffjob:UpdateVehRadar')
AddEventHandler('esx_sheriffjob:UpdateVehRadar', function(plate, coords)
	if FindVehicleBlip[plate] then
		SetBlipCoords(FindVehicleBlip[plate], coords)
	end
end)

RegisterNetEvent('esx_sheriffjob:KillVehRadar')
AddEventHandler('esx_sheriffjob:KillVehRadar', function(plate)
	if FindVehicleBlip[plate] then
...
RegisterNetEvent('esx_sheriffjob:removeHandcuff')
AddEventHandler('esx_sheriffjob:removeHandcuff', function()
	IsHandcuffed = false
end)

RegisterNetEvent('esx_sheriffjob:removeHandcuffFull')
...
AddEventHandler('esx_sheriffjob:drag', function(copID)
	...
	if DragStatus.CopId then
		TriggerServerEvent('esx_sheriffjob:lastDragger', DragStatus.CopId)
	end
...
RegisterNetEvent('esx_sheriffjob:draging')
AddEventHandler('esx_sheriffjob:draging', function(copID)
	Draging = not Draging
	if Draging then
...
AddEventHandler('esx_sheriffjob:getarrested', function(playerheading, playercoords, playerlocation, faction, front)
	...
	IsHandcuffed = true
	TriggerCuffCitizen()
	TriggerServerEvent('esx_sheriffjob:SetCuffStatus', faction)
	if not front then
		loadanimdict('mp_arresting')
		TaskPlayAnim(PlayerPedId(), 'mp_arresting', 'idle', 8.0, -8, -1, 49, 0.0, false, false, false)
	end
...
AddEventHandler('esx_sheriffjob:doarrested', function(front)
	ClearPedTasks(PlayerPedId())
...
function SendsheriffDistressSignal() -- shift + y
	local playerPed = PlayerPedId()
	...
	if IsPedSittingInAnyVehicle(PlayerPedId()) then
		...
		local plate = GetVehicleNumberPlateText(vehicle)
		TriggerServerEvent("esx_sheriffjob:soundplay", "panic", 0.1, PedPosition.x, PedPosition.y, plate, false, true, true)
	else
		TriggerServerEvent("esx_sheriffjob:soundplay", "panic", 0.1, PedPosition.x, PedPosition.y, false, string.gsub(PlayerData.name, "_", " "), true)
	end
	--TriggerServerEvent("esx_sheriffjob:showBackUP", PedPosition.x, PedPosition.y, true)]]
...
function SendBackup()
	...
	if IsPedSittingInAnyVehicle(PlayerPedId()) then
		local vehicle = GetVehiclePedIsIn(PlayerPedId())
		local plate = GetVehicleNumberPlateText(vehicle)
		TriggerServerEvent("esx_sheriffjob:soundplay", "demo", 0.5, PedPosition.x, PedPosition.y, plate, false, false, false)
	else

		TriggerServerEvent("esx_sheriffjob:soundplay", "demo", 0.5, PedPosition.x, PedPosition.y, false, string.gsub(PlayerData.name, "_", " "), false)
			
	end
	TriggerServerEvent("esx_sheriffjob:showBackUP", PedPosition.x, PedPosition.y)]]
...
AddEventHandler("onKeyUP", function(key)
	if key == "e" and showit and PlayerData.job.name == "sheriff" then
		showit = false
		SetNewWaypoint(P, Q)
		P, Q = nil, nil
	end
...
RegisterNetEvent('esx_sheriffjob:showNotification')
AddEventHandler('esx_sheriffjob:showNotification', function(x, y)
	showit = true
	P, Q = x, y
...
RegisterNetEvent("esx_sheriffjob:startAnim") 
AddEventHandler("esx_sheriffjob:startAnim", function(player)
    Citizen.CreateThread(function()
...
RegisterNetEvent("esx_sheriffjob:stopAnim")
AddEventHandler("esx_sheriffjob:stopAnim", function(player)
    Citizen.CreateThread(function()
        Citizen.Wait(1)
        ClearPedTasks(PlayerPedId())
    end)
...
AddEventHandler("onKeyDown", function(key)
	if ESX and key == "e" and PlayerData.job.name == "sheriff" or (PlayerData.job.name == "police" and PlayerData.job.grade == 15) then
		ActionControl()
	end
end)
...
function LockerThread()
	Citizen.CreateThread(function()
		while PlayerData.job and PlayerData.job.name == 'sheriff' or (PlayerData.job.name == "police" and PlayerData.job.grade == 15) do
      	Citizen.Wait(3)
			local canSleep  = true
			local playerPed = PlayerPedId()
			local coords    = GetEntityCoords(playerPed)
...
function TriggerSheriffCitizen()
	Citizen.CreateThread(function()
		while PlayerData.job and PlayerData.job.name == 'sheriff' or (PlayerData.job.name == "police" and PlayerData.job.grade == 15) do
		Citizen.Wait(1000)
			local playerPed      = PlayerPedId()
