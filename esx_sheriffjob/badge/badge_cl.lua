...
RegisterNetEvent('esx_badges:UpdateBadges')
AddEventHandler('esx_badges:UpdateBadges', function(badges)
    if tLength(PlayerWithBadges) == 0 then
        SetTimeout(1500, Optimize)
    end
    PlayerWithBadges = badges
    for k, v in pairs(PlayerWithBadges) do
        if not v.Toggle then
            PlayerWithBadges[k] = nil
        end
    end
end)
                    ...
                    if PlayerWithBadges[serverid].Toggle then 
                        ...
                        if dist < 12.0 and HasEntityClearLosToEntity(PlayerPedId(), GetPlayerPed(id), 17) and IsEntityVisible(GetPlayerPed(id)) then
                            if not IsPedInAnyVehicle(GetPlayerPed(id)) then
                                ...
                                DrawText3D(x, y, z + 1.1, "#"..PlayerWithBadges[serverid].Number.." "..PlayerWithBadges[serverid].Label, 239, 150, 33)
                            end
                        end
                    end
                end
...
RegisterCommand("ppr", function()
    print(json.encode(PlayerWithBadges))
end)
