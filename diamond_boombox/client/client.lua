...
AddEventHandler('wasabi_boombox:useBoombox', function()
    ...
    local hash = loadModel('qua_jblspeaker')
...
AddEventHandler('wasabi_boombox:interact', function()
    ...
    local radio = GetClosestObjectOfType(pedCoords, 5.0, `qua_jblspeaker`, false)
...
AddEventHandler('wasabi_boombox:saveSong', function()
    local input = lib.inputDialog('Save Song', {'Name', 'Link'})
...
AddEventHandler('wasabi_boombox:playMenu', function(data)
    ...
    if data.type == 'play' then
        local keyboard = lib.inputDialog('Play Music', {'URL','Distance (Max 10)', 'Volume (1-100)'})
        if keyboard then
            if keyboard[1] and tonumber(keyboard[2]) and tonumber(keyboard[2]) <= 10 and tonumber(keyboard[3]) and tonumber(keyboard[3]) <= 100 then
    ...
    elseif data.type == 'distance' then
        local keyboard = lib.inputDialog('Change Distance', {'Distance (Max 10)'})
        if keyboard then
            if tonumber(keyboard[1]) and tonumber(keyboard[1]) <= 10 then
