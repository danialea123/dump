...
window.addEventListener("message", function (event) {
  ...
  if (data.type == "ADD_NOTIFICATION") {
    ...
    let textExist = StackedText.find((el) => el.text == data.text && el.type == data.notifytype)
    if(textExist){
      textExist.count += 1
      $('#notify-'+textExist.id+' .stacked').show()
      $('#notify-'+textExist.id+' .stacked').css('display', 'flex')
      $('#notify-'+textExist.id+' .stacked').text(textExist.count)
    }else{
