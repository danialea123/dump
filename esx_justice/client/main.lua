...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	if PlayerData.job.name == 'justice' then
		TriggerjusticeCitizen()
		LockerThread()
	end
end)
...
function ActionControl()
	if CurrentAction ~= nil then
		...
		elseif CurrentAction == 'menu_boss_actions' then
			...
			TriggerEvent('esx_society:openBossMenu', 'justice', function(data, menu)
				menu.close()
...
AddEventHandler("onKeyUP", function(key)
	if key == "f6" then
		if PlayerData.job.name == "justice" and not isDead and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'justice_actions') and not ESX.GetPlayerData().isSentenced then
			OpenjusticeActionsMenu()
		end
...
AddEventHandler("onMultiplePress", function(keys)
	if keys["lshift"] and keys["y"] then
		if not isDead and PlayerData.job.name == "justice" then
			if GetGameTimer() - Timer > 10000 then
				SendjusticeDistressSignal()
				Timer = GetGameTimer()
			else
				ESX.Alert("Spam Nakonid", "error")
			end
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = _U('justice_wear'),  value = 'justice_wear'	}
	}

	if ESX.GetPlayerData().job.ext == 'lawyer' then
		table.insert(elements, {label = "Lebas Vakil", value = 'lawyer_wear'})
		--table.insert(elements, {label = 'Lebas X-Ray', value = 'xray_wear'})
	elseif ESX.GetPlayerData().job.ext == 'judge' then
		table.insert(elements, {label = "Lebas Ghaazi", value = 'judge_wear'})
	elseif ESX.GetPlayerData().job.ext == 'marshal' then
		table.insert(elements, {label = "Lebas Marshal", value = 'marshal_wear'})
	elseif ESX.GetPlayerData().job.ext == 'prosecutor' then
		table.insert(elements, {label = "Lebas Daadsetan", value = 'prosecutor_wear'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'citizen_wear' then
			...
			return
		elseif data.current.value == 'justice_wear' then
			ESX.SetPedArmour(PlayerPedId(), 0.0)
		...
		elseif data.current.value == 'lawyer_wear' then
			--if (ESX.GetPlayerData().job.ext == 'swat') or (ESX.GetPlayerData().job.ext == 'swat_manager') or (ESX.GetPlayerData().job.ext == 'iaa') or (ESX.GetPlayerData().job.ext == 'GND') then
				TriggerEvent('skinchanger:getSkin', function(skin)
					ESX.TriggerServerCallback("esx_society:HandleLawyerClothe", function(clothe)
			...
			--end
		elseif data.current.value == 'marshal_wear' then
			--if (ESX.GetPlayerData().job.ext == 'swat') or (ESX.GetPlayerData().job.ext == 'swat_manager') or (ESX.GetPlayerData().job.ext == 'iaa') or (ESX.GetPlayerData().job.ext == 'GND') then
				TriggerEvent('skinchanger:getSkin', function(skin)
					ESX.TriggerServerCallback("esx_society:HandleMarshalClothe", function(clothe)
						TriggerEvent('skinchanger:loadClothes', skin, json.decode(clothe))
						ESX.SetPedArmour(PlayerPedId(), 100.0)
					end, PlayerData.job.name, PlayerData.job.grade, tonumber(skin["sex"]))
				end)
		elseif data.current.value == 'judge_wear' then
			TriggerEvent('skinchanger:getSkin', function(skin)
				ESX.TriggerServerCallback("esx_society:HandleJudgeClothe", function(clothe)
					TriggerEvent('skinchanger:loadClothes', skin, json.decode(clothe))
					ESX.SetPedArmour(PlayerPedId(), 100.0)
				end, PlayerData.job.name, PlayerData.job.grade, tonumber(skin["sex"]))
			end)
		elseif data.current.value == "prosecutor_wear" then
			TriggerEvent('skinchanger:getSkin', function(skin)
				ESX.TriggerServerCallback("esx_society:HandleProsecutorClothe", function(clothe)
					TriggerEvent('skinchanger:loadClothes', skin, json.decode(clothe))
					ESX.SetPedArmour(PlayerPedId(), 100.0)
				end, PlayerData.job.name, PlayerData.job.grade, tonumber(skin["sex"]))
			end)
...
function SpawnVehicle(vehicle, plate, coords, heading)
	...
	if not DoesEntityExist(shokol) then
		TriggerServerEvent('esx_justice:saveVehicle', plate, true)
		ESX.Game.SpawnVehicle(vehicle.model, {
			...
			TriggerServerEvent('savedVehicles', 'justice', VehToNet(callback_vehicle))
			Wait(2000)
			exports["LegacyFuel"]:SetFuel(callback_vehicle, 100.0)
			SetVehicleMaxMods(callback_vehicle, true)
		end)
	else
...
function OpenVehicleSpawnerMenu(station, partNum)
	local vehicles = Config.justiceStations[station].Vehicles
	ESX.UI.Menu.CloseAll()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'car_plate', {
				...
				if data3.value == nil then
					...
					print()
					ESX.TriggerServerCallback('esx:GetVehicleByPlate', function(veh)
						ESX.TriggerServerCallback('esx_justice:callbackVehicle', function(status)
							if status then
					ESX.Alert('In Pelak Ghablan Vared Shode Va Dar Hal Hazer Motalegh Be Yek Nafar Mibashad!', "info")
							else
...
function OpenHeliSpawnerMenu(station, partNum)
	local helicopters = Config.justiceStations[station].Helicopters
	ESX.UI.Menu.CloseAll()
	if not IsAnyVehicleNearPoint(helicopters[partNum].SpawnPoint.x, helicopters[partNum].SpawnPoint.y, helicopters[partNum].SpawnPoint.z,  3.0) then
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'heli_spawner',
		{
			...
			align    = 'top-left',
			elements = {
				{label = 'Heli - 1', value = 'buzzard2'},
			}
		}, function(data, menu)
			menu.close()
			if data.current.value == 'polmash' then
				ESX.Game.SpawnVehicle('polmash', helicopters[partNum].SpawnPoint, helicopters[partNum].Heading, function(vehicle)
					SetVehicleModKit(vehicle, 0)
					SetVehicleLivery(vehicle, 0)
					Citizen.Wait(2000)
					TriggerServerEvent('savedVehicles', 'justice', VehToNet(vehicle))
...
function OpenjusticeActionsMenu()
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'justice_actions', {
		title    = 'justice',
		align    = 'top-left',
		elements = {
			...
			'default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'give_weap_license' then
						ChooseFromNearbyPlayers(2, function(target)
							TriggerServerEvent('esx_justice:GiveWeaponLicese', target)
						end)
		...
		elseif data.current.value == 'mini_jail_menu' then
			TriggerEvent("esx-qalle-jail:openMiniJailMenu", "justice")
		...
		elseif data.current.value == "warning" then
			...
			function(data, menu)
				if data.current.value == "1" then
					ExecuteCommand("mp شما تحت محاصره ی شریف لوس سانتوس هستید, تسلیم شوید, اخطار اول")
					menu.close()
				elseif data.current.value == "2" then
					ExecuteCommand("mp شما تحت محاصره ی شریف لوس سانتوس هستید, تسلیم شوید, اخطار دوم")
					menu.close()
				elseif data.current.value == "3" then
					ExecuteCommand("mp اخطار آخر, در صورت عدم همکاری تا ۱۰ ثانیه ی دیگر حکم تیر صادر خواهد شد")
					menu.close()
				else
					ExecuteCommand("mp شریف لوس سانتوس صحبت می کنه, ماشین رو بزن بغل و خاموش کن")
					menu.close()
				end
			end, function(data, menu)
				menu.close()
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'custom_fine_number', {
				...
				if amount ~= nil then
					if amount <= 100000 then
						menu2.close()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_justice', _U('fine_total', reason), amount, true)
						TriggerServerEvent('DiscordBot:ToDiscord', 'jobsfine', 'JOBS FINE LOG', '***Justice Fine***\n```css\n[FROM]: ('..GetPlayerServerId(PlayerId())..') '..PlayerData.name..'\n[TO]: ('..GetPlayerServerId(player)..') '..GetPlayerName(player)..'\n[AMOUNT]: '..ESX.Math.GroupDigits(amount)..'\n[REASON]: '..reason..'```', 'user', GetPlayerServerId(PlayerId()), true, false)
					else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_justice:getVehicleFromPlate', function(owner, found)
				if found then
...
function OpenVehicleInfosMenu(vehicleData)
	...
	ESX.TriggerServerCallback('esx_justice:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_justice:getArmoryWeapons', function(weapons)
			...
			if not AccessAll then
				if Guns ~= nil then
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
						...
						ESX.TriggerServerCallback('esx_justice:removeArmoryWeapon', function(k)
							if k then 
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open(
		...
		ESX.TriggerServerCallback('esx_justice:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu(station)
	ESX.TriggerServerCallback('esx_justice:getArmoryWeapons', function(weapons)
		local elements = {}
		for i=1, #Config.justiceStations[station].AuthorizedWeapons, 1 do
			local weapon = Config.justiceStations[station].AuthorizedWeapons[i]
			local count  = 0
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons',
		{
			...
			ESX.TriggerServerCallback('esx_justice:buy', function(hasEnoughMoney)
...
function OpenBuyStockMenu(station)
	ESX.TriggerServerCallback('esx_justice:getStockItems', function(inventory)
		local elements = {}

		for i=1, #Config.justiceStations[station].AuthorizedItems, 1 do
			local item = Config.justiceStations[station].AuthorizedItems[i]
			local count  = 0
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_stock', {
			...
			ESX.TriggerServerCallback('esx_justice:buy', function(hasEnoughMoney)
				if hasEnoughMoney then
					TriggerServerEvent('esx_justice:addStockItems', data.current.value)
					OpenBuyStockMenu(station)
				else
					ESX.Alert(_U('not_enough_money'))
				end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
					...
					if count == nil then
						...
						TriggerServerEvent('esx_justice:getStockItem', itemName, count)
						Citizen.Wait(300)
						OpenGetStocksMenu()
					end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_justice:getPlayerInventory', function(inventory)
		...
		function(data, menu)
			...
			function(data2, menu2)
				...
				menu.close()
				if string.find(itemName, "casino") ~= nil then return end
				TriggerServerEvent('esx_justice:putStockItems', itemName, count)
...
AddEventHandler('esx_justice:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_justice:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

RegisterNetEvent('esx_justice:AddBlipForNumber')
AddEventHandler('esx_justice:AddBlipForNumber', function(target, name, job)
	if (PlayerData.job.name == job and PlayerData.job.grade > 0) then
		ESX.Alert("Moqeyiate "..(name):gsub("_", " ").." Roye ~g~Radar~s~ Moshakhas Shod!")
		TriggerjusticeRadar(target)
	end
end)

RegisterNetEvent('esx_justice:AddBlipForVehicle')
AddEventHandler('esx_justice:AddBlipForVehicle', function(plate, job)
	if (PlayerData.job.name == job and PlayerData.job.grade > 0) then
		TriggerjusticeVehicleRadar(plate)
	end
end)

local FindNumberBlip = {}
function TriggerjusticeRadar(target)
...
function TriggerjusticeVehicleRadar(plate)
...
RegisterNetEvent('esx_justice:UpdateRadar')
AddEventHandler('esx_justice:UpdateRadar', function(target, coords)
	if FindNumberBlip[target] then
		SetBlipCoords(FindNumberBlip[target], coords)
	end
end)

RegisterNetEvent('esx_justice:KillRadar')
AddEventHandler('esx_justice:KillRadar', function(target)
	if FindNumberBlip[target] then
...
RegisterNetEvent('esx_justice:UpdateVehRadar')
AddEventHandler('esx_justice:UpdateVehRadar', function(plate, coords)
	if FindVehicleBlip[plate] then
		SetBlipCoords(FindVehicleBlip[plate], coords)
	end
end)

RegisterNetEvent('esx_justice:KillVehRadar')
AddEventHandler('esx_justice:KillVehRadar', function(plate)
	if FindVehicleBlip[plate] then
...
RegisterNetEvent('esx_justice:removeHandcuff')
AddEventHandler('esx_justice:removeHandcuff', function()
	IsHandcuffed = false
end)

RegisterNetEvent('esx_justice:removeHandcuffFull')
AddEventHandler('esx_justice:removeHandcuffFull', function()

	local playerPed = PlayerPedId()
	
	IsHandcuffed = false
	TriggerServerEvent('esx_justice:SetCuffStatus', false)
	...
	TriggerEvent("esx_justice:removeHandcuff")
end)

RegisterNetEvent('esx_justice:unrestrain')
AddEventHandler('esx_justice:unrestrain', function()
...
RegisterNetEvent('esx_justice:drag')
AddEventHandler('esx_justice:drag', function(copID)
	if not IsHandcuffed then
		return
	end
	if DragStatus.CopId then
		TriggerServerEvent('esx_justice:lastDragger', DragStatus.CopId)
	end
	DragStatus.IsDragged = not DragStatus.IsDragged
	DragStatus.CopId     = tonumber(copID)
end)

RegisterNetEvent('esx_justice:lastDragger')
AddEventHandler('esx_justice:lastDragger', function()
	Draging = false
end)

RegisterNetEvent('esx_justice:draging')
AddEventHandler('esx_justice:draging', function(copID)
	Draging = not Draging
	if Draging then
...
RegisterNetEvent('esx_justice:putInVehicle')
AddEventHandler('esx_justice:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_justice:OutVehicle')
AddEventHandler('esx_justice:OutVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('esx_justice:getarrested')
AddEventHandler('esx_justice:getarrested', function(playerheading, playercoords, playerlocation, faction, front)
	playerPed = PlayerPedId()
	SetCurrentPedWeapon(playerPed, GetHashKey('WEAPON_UNARMED'), true) -- unarm player
	ESX.UI.Menu.CloseAll()
	...
	IsHandcuffed = true
	TriggerCuffCitizen()
	TriggerServerEvent('esx_justice:SetCuffStatus', faction)
	if not front then
		loadanimdict('mp_arresting')
		TaskPlayAnim(PlayerPedId(), 'mp_arresting', 'idle', 8.0, -8, -1, 49, 0.0, false, false, false)
	end
...
RegisterNetEvent('esx_justice:doarrested')
AddEventHandler('esx_justice:doarrested', function(front)
	ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('esx_justice:douncuffing')
AddEventHandler('esx_justice:douncuffing', function()
	ClearPedTasks(PlayerPedId())
	local Idies = ESX.Game.GetPlayersServerIdInArea(GetEntityCoords(PlayerPedId()), 5.0)
...
RegisterNetEvent('esx_justice:getuncuffed')
AddEventHandler('esx_justice:getuncuffed', function(playerheading, playercoords, playerlocation)
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	ESX.SetEntityCoords(PlayerPedId(), x, y, z)
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_justice:unrestrain')
end)
...
function ToggleVehicleLock()
	...
	if has_value("justice", xPlayer.job.name) then
...
function SendjusticeDistressSignal() -- shift + y
	local playerPed = PlayerPedId()
	PedPosition		= GetEntityCoords(playerPed)
	ExecuteCommand("me Dastesho Mibare Samt Radio Va Dokme Panic Alarm Ro Feshar Mide")
	ESX.Alert(_U('distress_sent'), "check")
	--[[local Idies = ESX.Game.GetPlayersServerIdInArea(GetEntityCoords(PlayerPedId()), 5.0)
	TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 1.0, 'panic', 0.5)

	if IsPedSittingInAnyVehicle(PlayerPedId()) then
		local vehicle = GetVehiclePedIsIn(PlayerPedId())
		local plate = GetVehicleNumberPlateText(vehicle)
		TriggerServerEvent("esx_justice:soundplay", "panic", 0.2, PedPosition.x, PedPosition.y, plate, false, true, true)
	else
		TriggerServerEvent("esx_justice:soundplay", "panic", 0.2, PedPosition.x, PedPosition.y, false, string.gsub(PlayerData.name, "_", " "), true)
	end
	TriggerServerEvent("esx_justice:showBackUP", PedPosition.x, PedPosition.y, true)]]
...
function SendBackup()
	...
	ESX.Alert(_U('distress_sent'), "check")
	--[[local Idies = ESX.Game.GetPlayersServerIdInArea(GetEntityCoords(PlayerPedId()), 5.0)
	TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 1.0, 'demo', 0.5)

	if IsPedSittingInAnyVehicle(PlayerPedId()) then
		local vehicle = GetVehiclePedIsIn(PlayerPedId())
		local plate = GetVehicleNumberPlateText(vehicle)
		TriggerServerEvent("esx_justice:soundplay", "demo", 0.2, PedPosition.x, PedPosition.y, plate, false, false, false)
	else

		TriggerServerEvent("esx_justice:soundplay", "demo", 0.2, PedPosition.x, PedPosition.y, false, string.gsub(PlayerData.name, "_", " "), false)
			
	end
	TriggerServerEvent("esx_justice:showBackUP", PedPosition.x, PedPosition.y)]]
...
AddEventHandler("onKeyUP", function(key)
	if key == "e" and showit and PlayerData.job.name == "justice" then
		showit = false
		SetNewWaypoint(P, Q)
		P, Q = nil, nil
	end
...
RegisterNetEvent('esx_justice:showNotification')
AddEventHandler('esx_justice:showNotification', function(x, y)
	showit = true
	P, Q = x, y
...
RegisterNetEvent("esx_justice:startAnim") 
AddEventHandler("esx_justice:startAnim", function(player)
    Citizen.CreateThread(function()
...
RegisterNetEvent("esx_justice:stopAnim")
AddEventHandler("esx_justice:stopAnim", function(player)
    Citizen.CreateThread(function()
        Citizen.Wait(1)
        ClearPedTasks(PlayerPedId())
    end)
...
-- Create blips
CreateThread(function()
	for k,v in pairs(Config.justiceStations) do
...
AddEventHandler("onKeyDown", function(key)
	if key == "e" and PlayerData.job.name == "justice" then
		ActionControl()
	end
end)
...
function LockerThread()
	Citizen.CreateThread(function()
    	while PlayerData.job and PlayerData.job.name == 'justice' do
      	Citizen.Wait(3)
			local canSleep  = true
			local playerPed = PlayerPedId()
			local coords    = GetEntityCoords(playerPed)
...
function TriggerjusticeCitizen()
	Citizen.CreateThread(function()
		while PlayerData.job and PlayerData.job.name == 'justice' do
		Citizen.Wait(1000)
			local playerPed      = PlayerPedId()
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				if (LastStation ~= nil and LastPart ~= nil and LastPartNum ~= nil) and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) then
					TriggerEvent('esx_justice:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_justice:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
